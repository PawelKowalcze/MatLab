...
f = [ 1500  2500  3500 6000];    % liczba powtorzen na sekunde skladowych sinusoid
d = [ 50     100     250   1  ];    % tlumienie skladowych
A = [ 2  3   4    0.77 ];    % amplituda skladowych
...
for k = 1 : K                 % generacja i akumulacja kolejnych sinusoid  
    x =  x + A(k) * exp(-d(k)*(0:N-1)*dt) .* cos(2*pi*f(k)*(0:N-1)*dt + pi*rand(1,1)) + randn();
end
