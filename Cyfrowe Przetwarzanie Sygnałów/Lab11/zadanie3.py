...
if data.ndim > 1:  # jeśli stereo, bierzemy tylko jeden kanał
    data = data[:,0]

start = int(fs * 10)  # np. od 10 sekundy
end = start + int(fs * 3)  # 3-sekundowy fragment
x = data[start:end]

def kodowanie_podpasmowe_python(x, M, q_bits):
    """
    Uproszczona wersja podpasmowego kodowania:
    - dzieli sygnał na M podpasma (przez okienkowanie i FFT),
    - kwantuje każde podpasmo z podaną liczbą bitów q_bits (stałe lub lista),
    - rekonstruuje sygnał,
    - zwraca zrekonstruowany sygnał i średnią liczbę bitów na próbkę.
    """
    N = len(x)
    if isinstance(q_bits, int):
        q_bits = np.full(M, q_bits)
    elif len(q_bits) < M:
        q_bits = np.pad(q_bits, (0, M - len(q_bits)), 'edge')
    else:
        q_bits = q_bits[:M]

    # Parametry analizy - użyjemy STFT z M "pasami"
