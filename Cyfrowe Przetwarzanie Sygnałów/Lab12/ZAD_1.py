...
# Wyzeruj połowę największych współczynników (poza DC)
flat = np.abs(dct_img1).flatten()
sorted_indices = np.argsort(flat)[::-1]
keep = sorted_indices[:len(flat)//2]
mask = np.zeros_like(dct_img1)
mask_flat = mask.flatten()
mask_flat[keep] = 1
mask = mask_flat.reshape(dct_img1.shape)
dct_reduced = dct_img1 * mask
...
show_image(np.log(np.abs(dct_img2)+1), "DCT2 of cameraman")

# a) Wyzeruj współczynniki wysokich częstotliwości
threshold_freq = 50
mask = np.zeros_like(dct_img2)
mask[:threshold_freq, :threshold_freq] = 1
dct_lowpass = dct_img2 * mask
img_lowpass = idct2(dct_lowpass)
show_image(img_lowpass, "Low Frequencies Only")
...
show_image(img_thresh, "Values > threshold (abs)")
...
show_image(reconstructed, "Single Basis Image (2,10)")

# 4
img_zagadka = cv2.imread("im2.png", cv2.IMREAD_GRAYSCALE)
dct_zagadka = dct2(img_zagadka.astype(float))
show_image(img_zagadka, "Zagadka")
show_image(np.log(np.abs(dct_zagadka)+1), "DCT2 of Zagadka")
