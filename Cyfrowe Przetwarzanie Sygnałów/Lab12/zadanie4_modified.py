...
def quantize_zone_coding(block, z_coefficients):
    """
    Pozostawia tylko z_coefficients w okolicy DC (lewym górnym rogu).
    Pozostałe współczynniki są zerowane.
    """
    quantized_block = np.zeros_like(block)
    rows, cols = block.shape

    # Kopiujemy z_coefficients z lewego górnego rogu
    # Upewniamy się, że nie wychodzimy poza wymiary bloku
    for i in range(min(z_coefficients, rows)):
        for j in range(min(z_coefficients - i, cols)):  # zachowujemy "trójkątny" obszar
...
def quantize_threshold_coding(block, t_threshold):
    """
    Pozostawia tylko współczynniki o wartości bezwzględnej większej niż t_threshold.
    Pozostałe współczynniki są zerowane.
    """
    quantized_block = np.zeros_like(block)
    quantized_block[np.abs(block) > t_threshold] = np.round(block[np.abs(block) > t_threshold])
    return quantized_block
...
def main():
    ...
    # Parametry dla standardowej kwantyzacji
    qs = [10, 20, 40, 80, 120]

    # Parametry dla zone-coding (liczba współczynników do zachowania)
    zs = [1, 3, 5, 8,
          10]  # Liczba współczynników w lewym górnym rogu (np. 1 dla samego DC, 3 dla 2x2 z pominięciem jednego)

    # Parametry dla threshold-coding (wartość progowa)
    ts = [5, 10, 20, 50, 100]
